  ...
  methods: {
    ...
    process(x) {
      ...
      if(stateTable.length) {
        if (stateTable[0].label.length >= 17) {
          x.state = (
            <el-tooltip class="item" effect="dark" content={stateTable[0].label} placement="top" open-delay="300">
          ...
          x.state = (<router-link to={`/status/${x.id}`} style={`color: ${stateTable[0].color}; white-space: nowrap;`} class="text-bold"><i class={stateTable[0].icon} /> {stateTable[0].label}</router-link>);
        }
      } else {
